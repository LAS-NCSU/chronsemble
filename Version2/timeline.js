        ...
        var labelDefs = (bandName === "naviBand") ? [
            ...
            [["scrubWindow", "bandRangeLabel", 0, labelWidth / 3,
                ...
                ["reference", "bandReferenceLabel", (band.w - labelWidth) / 2,
                    ...
                    function (min, max) {
                        return ((max.getUTCFullYear() + max.getUTCMonth() / 12) - (min.getUTCFullYear() + min.getUTCMonth() / 12)) / 2 + min.getUTCFullYear() + min.getUTCMonth() / 12;
                    },
...
function timeline(domTimelineElement, domSpatioFlowElement, domInfoFlowElement) {
    ...
    function scrubberValue(scrubberPart) {
        ...
        dataInFile.forEach(function (value) {
            ...
            coverflowValues.push(arrayObject);
        });
        ...
        generateCoverflow(coverflowValues, scrubberWindowRange, centreValue);
    }

    function generateCoverflow(coverflowValues, scrubberWindowRange, centreValue) {
        ...
        var valuesOnTheCoverflow = [];
        var result = [];
        coverflowValues.forEach(function (value) {
            ...
            if ((startDateOfFile >= start && endDateOfFile <= end) || (startDateOfFile < start && (endDateOfFile > start && endDateOfFile <= end)) || ((startDateOfFile >= start && startDateOfFile < end) && endDateOfFile > end)) {
                valuesOnTheCoverflow.push(value);
        ...
        var centre = displayCoverflow(valuesOnTheCoverflow, result[0]);
    }


    function displayCoverflow(valuesOnTheCoverflow, centreDisplayValue) {
        ...
        var svgFlow = d3.select("#svgInfoFlow");
        var texts = svgFlow.selectAll("text")
            .data(" ")
            .enter();
