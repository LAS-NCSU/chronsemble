...
function populateData(domElement, random, source) {
    d3.csv(source, function (dataset) {
        timelineAddition(domElement, random, source).data(dataset).band("mainBand", 0.82).band("naviBand", 0.08).xAxis("mainBand").tooltips("mainBand").xAxis("naviBand").labels("mainBand").labels("naviBand").brush("naviBand", ["mainBand"]).redraw();
    });
}
...
function timelineAddition(domElement, random, source) {
    ...
    var buttonMargin = { top: 0, right: 0, bottom: 0, left: 0 },
        yAxis = 410,
        buttonWidth = 20,
        buttonHeight = 500 - yAxis;

    // global timelineAddition variables
    var timelineAddition = {},
        // The timelineAddition
    data = {},
        // Container for the data
    components = [],
        // All the components of the timelineAddition for redrawing
    bandGap = 25,
        // Arbitray gap between to consecutive bands
    bands = {},
        // Registry for all the bands in the timelineAddition
    bandY = 0,
    ...
    var dom = d3.select(domElement);
    var svg = dom.select('#philosophers.csv').append("svg").attr("class", "svg").attr("id", res[pos]).attr("width", outerWidth).attr("height", outerHeight).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    svg.append("clipPath").attr("id", "chart-area1").append("rect").attr("width", width).attr("height", height);

    var chart = svg.append("g").attr("class", "chart").attr("clip-path", "url(#chart-area1)");
    ...
    timelineAddition.data = function (items) {
        ...
        data.items.forEach(function (item) {
            ...
            // The timelineAddition never reaches into the future.
    ...
    timelineAddition.band = function (bandName, sizeFactor) {
        ...
        band.itemHeight = band.trackHeight * 0.8, band.parts = [], band.instantWidth = 100; // arbitray value
    ...
    timelineAddition.labels = function (bandName) {
        ...
        var labelDefs = [["start", "bandMinMaxLabel", 0, 4, function (min, max) {
            return toYear(min);
        }, "Start of the selected interval", band.x + 30, labelTop], ["end", "bandMinMaxLabel", band.w - labelWidth, band.w - 4, function (min, max) {
            return toYear(max);
        }, "End of the selected interval", band.x + band.w - 152, labelTop], ["middle", "bandMidLabel", (band.w - labelWidth) / 2, band.w / 2, function (min, max) {
    ...
    timelineAddition.tooltips = function (bandName) {
        ...
        function showTooltip(d) {
            ...
            var x = event.pageX < band.x + band.w / 2 ? event.pageX + 10 : event.pageX - 110,
                y = event.pageY < band.y + band.h / 2 ? event.pageY + 30 : event.pageY - 30;
    ...
    timelineAddition.xAxis = function (bandName, orientation) {
        ...
        components.push(xAxis); // for timelineAddition.redraw
    ...
    timelineAddition.brush = function (bandName, targetNames) {
    ...
    timelineAddition.redraw = function () {
