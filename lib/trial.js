...
bandGap = 25,
    // Arbitrary gap between to consecutive bands
bands = {},
...
function init() {
    ...
    d3.csv(sourceFile, function (dataset) {
        timelineInit(domElement, sourceFile).data(dataset).band("mainBand", 0.82).band("naviBand", 0.08).xAxis("mainBand").tooltips("mainBand").xAxis("naviBand").labels("mainBand").labels("naviBand").brush("naviBand", ["mainBand"]).redraw();
    });
}
init();
...
function timelineInit(domElement, source) {
    ...
    */
    ...
    var svg = d3.select(domElement).append("svg").attr("class", "svg").attr("id", res[pos]).attr("width", outerWidth).attr("height", outerHeight).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    ...
    var button_svg = d3.select(domElement).append("svg").attr("class", "svg").attr("id", "buttonSvg").attr("y", yAxis).attr("width", buttonWidth).attr("height", buttonHeight).attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    var group1 = button_svg.append("g").attr("id", "group1");
    var fo1 = group1.append("foreignObject").attr("width", 20).attr("height", 20).attr("class", "fo").attr("y", 0);
    ...
    var fo2 = group2.append("foreignObject").attr("width", 20).attr("height", 20).attr("class", "fo").attr("y", 70);
